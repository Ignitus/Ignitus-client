...
import { IgnitusBlue, GreyLight } from '../../..';
...
export const Circle = styled('span', { shouldForwardProp: noForward })<
  CircleProps
>`
  ...
  ${props =>
    props.current && `box-shadow: inset 0px 0px 0px 0.1rem ${IgnitusBlue};`}
  border-radius: 50%;
  background-color: ${props => (props.fill ? IgnitusBlue : 'none')};
  transition: background-color 0.2s linear 0.1s,
    box-shadow 0.5s ease-in ${props => props.delay}s;
`;
...
export const Line = styled('span', { shouldForwardProp: noForward })<LineProps>`
  ...
  ${props =>
    props.fill ? `box-shadow: inset 100px 0 0 0 ${IgnitusBlue};` : null}
  margin: 0 0.5rem;
